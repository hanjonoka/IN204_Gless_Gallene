...
public:
    ...
    Rayon_t(Vector_t origine, Vector_t direction, std::vector<Objet_t*>* scene, std::vector<Objet_t*>* sources, int nb_rebond, bool diffusion)
    {
        this->nb_rebond = nb_rebond+1;
        init(origine, direction, scene, sources, diffusion);
    }

    void init(Vector_t origine, Vector_t direction, std::vector<Objet_t*>* scene, std::vector<Objet_t*>* sources, bool diffusion){
