...
public:
    ...
    Rayon_t(Vector_t origine, Vector_t direction, std::vector<Objet_t*>* scene, std::vector<Objet_t*>* sources)
    {
        init(origine, direction, scene, sources, NULL);
    }

    //rayon diffusé
    Rayon_t(Vector_t origine, Vector_t direction, std::vector<Objet_t*>* scene, std::vector<Objet_t*>* sources, int nb_rebond, Objet_t* source_diff)
    {
    ...
    Rayon_t(Vector_t origine, Vector_t direction, std::vector<Objet_t*>* scene, std::vector<Objet_t*>* sources, int nb_rebond, bool refracte)
    {
        this->nb_rebond = nb_rebond+1;
    ...
    void init(Vector_t origine, Vector_t direction, std::vector<Objet_t*>* scene, std::vector<Objet_t*>* sources, Objet_t* source_diff){
    ...
    Intersection_t* trouve_premier_obstacle(std::vector<Objet_t*>* scene)
    {
    ...
    void lancer_rayons()
    {
        ...
        if(intersect->object->has_interior){ //seulement si l'objet à un intérieur
